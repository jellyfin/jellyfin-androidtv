...
@Composable
fun SettingsScreensaverSleepTimeoutScreen() {
	...
	val options = getScreensaverSleepTimeoutOptions()
	...
	SettingsColumn {
		...
		items(options) { (duration, heading) ->
			ListButton(
				...
				trailingContent = { RadioButton(checked = screensaverInAppSleepTimeout == duration.inWholeMilliseconds) },
				onClick = {
					screensaverInAppSleepTimeout = duration.inWholeMilliseconds
					router.back()
				},
				modifier = Modifier.focusKey(
					"sleep_timeout_${duration.inWholeMilliseconds}",
					initialFocus = screensaverInAppSleepTimeout == duration.inWholeMilliseconds
				)
			)
		}
	}
