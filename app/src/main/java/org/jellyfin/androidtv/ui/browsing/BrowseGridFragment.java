...
public class BrowseGridFragment extends Fragment implements View.OnKeyListener {
  ...
  private final Lazy<PreferencesRepository> preferencesRepository =
      inject(PreferencesRepository.class);
  ...
  private final Lazy<CustomMessageRepository> customMessageRepository =
      inject(CustomMessageRepository.class);
  ...
  private double mCardFocusScale =
      1.15; // 115%, just a default we use the resource card_scale_focus otherwise
  private final int MIN_NUM_CARDS =
      5; // minimum number of visible cards we allow, this results in more empty space
  private final double CARD_SPACING_PCT =
      1.0; // 100% expressed as relative to the padding_left/top, which depends on the
  // mCardFocusScale and AspectRatio
  private final double CARD_SPACING_HORIZONTAL_BANNER_PCT =
      0.5; // 50% allow horizontal card overlapping for banners, otherwise spacing is too large
  private final int VIEW_SELECT_UPDATE_DELAY =
      250; // delay in ms until we update the top-row info for a selected item
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    mGridHeight =
        Math.round(display.heightPixels / getResources().getDisplayMetrics().density - 130.6f);
    ...
    mFolder =
        Json.Default.decodeFromString(
            BaseItemDto.Companion.serializer(), getArguments().getString(Extras.Folder));
    mParentId = mFolder.getId();
    mainTitle = mFolder.getName();
    libraryPreferences =
        preferencesRepository
            .getValue()
            .getLibraryPreferences(Objects.requireNonNull(mFolder.getDisplayPreferencesId()));
    ...
    sortOptions.clear();
    {
      sortOptions.put(
          0,
          new SortOption(getString(R.string.lbl_name), ItemSortBy.SORT_NAME, SortOrder.ASCENDING));
      sortOptions.put(
          1,
          new SortOption(
              getString(R.string.lbl_date_added), ItemSortBy.DATE_CREATED, SortOrder.DESCENDING));
      sortOptions.put(
          2,
          new SortOption(
              getString(R.string.lbl_premier_date),
              ItemSortBy.PREMIERE_DATE,
              SortOrder.DESCENDING));
      sortOptions.put(
          3,
          new SortOption(
              getString(R.string.lbl_rating), ItemSortBy.OFFICIAL_RATING, SortOrder.ASCENDING));
      sortOptions.put(
          4,
          new SortOption(
              getString(R.string.lbl_community_rating),
              ItemSortBy.COMMUNITY_RATING,
              SortOrder.DESCENDING));
      sortOptions.put(
          5,
          new SortOption(
              getString(R.string.lbl_critic_rating),
      ...
      if (mFolder.getCollectionType() == CollectionType.TVSHOWS) {
        ...
        sortOptions.put(
            6,
            new SortOption(
                getString(R.string.lbl_last_played), ItemSortBy.DATE_PLAYED, SortOrder.DESCENDING));
      }

      if (mFolder.getCollectionType() != null
          && mFolder.getCollectionType() == CollectionType.MOVIES) {
        sortOptions.put(
            7,
            new SortOption(
                getString(R.string.lbl_runtime), ItemSortBy.RUNTIME, SortOrder.ASCENDING));
      }
    }
  ...
  private void createGrid() {
    ...
    if (mGridViewHolder instanceof HorizontalGridPresenter.ViewHolder) {
      ...
      ViewGroup.MarginLayoutParams titleMargin =
          (ViewGroup.MarginLayoutParams) binding.title.getLayoutParams();
      ViewGroup.MarginLayoutParams clockMargin =
          (ViewGroup.MarginLayoutParams) binding.clock.getLayoutParams();
      mGridView.setPadding(
          titleMargin.getMarginStart(),
          mGridPaddingTop,
          clockMargin.getMarginEnd(),
          mGridPaddingTop); // prevent initial card cutoffs
  ...
  private SortOption getSortOption(ItemSortBy value) {
    ...
    return new SortOption(
        getString(R.string.lbl_bracket_unknown), ItemSortBy.SORT_NAME, SortOrder.ASCENDING);
  }
  ...
  public void setStatusText(String folderName) {
    ...
    if (filters == null || (!filters.isFavoriteOnly() && !filters.isUnwatchedOnly())) {
      ...
      text +=
          (filters.isUnwatchedOnly() ? getString(R.string.lbl_unwatched) : "")
              + " "
    ...
    text +=
        " "
            + getString(R.string.lbl_from)
            + " '"
            + folderName
            + "' "
            + getString(R.string.lbl_sorted_by)
            + " "
            + getSortOption(mAdapter.getSortBy()).name;
  ...
  public void updateCounter(int position) {
    if (mAdapter != null) {
      binding.counter.setText(
          MessageFormat.format("{0} | {1}", position, mAdapter.getTotalItems()));
    }
  }
  ...
  private double getCardWidthBy(final double cardHeight, ImageType imageType, BaseItemDto folder) {
    switch (imageType) {
      case POSTER:
        ...
        if (fType == BaseItemKind.AUDIO
            || fType == BaseItemKind.GENRE
            || fType == BaseItemKind.MUSIC_ALBUM
            || fType == BaseItemKind.MUSIC_ARTIST
            || fType == BaseItemKind.MUSIC_GENRE) {
          return cardHeight;
        } else if (fType == BaseItemKind.COLLECTION_FOLDER
            && CollectionType.MUSIC.equals(folder.getCollectionType())) {
          return cardHeight;
        } else {
  ...
  private void setDefaultGridRowCols(PosterSize posterSize, ImageType imageType) {
    ...
    if (mGridPresenter instanceof VerticalGridPresenter) {
      ...
      switch (posterSize) {
        case SMALLEST:
          numCols =
              imageType.equals(ImageType.BANNER) ? 6 : imageType.equals(ImageType.THUMB) ? 11 : 15;
          break;
        case SMALL:
          numCols =
              imageType.equals(ImageType.BANNER) ? 5 : imageType.equals(ImageType.THUMB) ? 9 : 13;
          break;
        case MED:
          numCols =
              imageType.equals(ImageType.BANNER) ? 4 : imageType.equals(ImageType.THUMB) ? 7 : 11;
          break;
        case LARGE:
          numCols =
              imageType.equals(ImageType.BANNER) ? 3 : imageType.equals(ImageType.THUMB) ? 5 : 7;
          break;
        case X_LARGE:
          numCols =
              imageType.equals(ImageType.BANNER) ? 2 : imageType.equals(ImageType.THUMB) ? 3 : 5;
          break;
        default:
      ...
      switch (posterSize) {
        case SMALLEST:
          numRows =
              imageType.equals(ImageType.BANNER) ? 13 : imageType.equals(ImageType.THUMB) ? 7 : 5;
          break;
        case SMALL:
          numRows =
              imageType.equals(ImageType.BANNER) ? 11 : imageType.equals(ImageType.THUMB) ? 6 : 4;
          break;
        case MED:
          numRows =
              imageType.equals(ImageType.BANNER) ? 9 : imageType.equals(ImageType.THUMB) ? 5 : 3;
          break;
        case LARGE:
          numRows =
              imageType.equals(ImageType.BANNER) ? 7 : imageType.equals(ImageType.THUMB) ? 4 : 2;
          break;
        case X_LARGE:
          numRows =
              imageType.equals(ImageType.BANNER) ? 5 : imageType.equals(ImageType.THUMB) ? 2 : 1;
          break;
        default:
          throw new IllegalStateException("Unexpected value: " + mPosterSizeSetting);
      }
  ...
  private void setAutoCardGridValues() {
    ...
    if (numRows > 0) {
      ...
      spacingVerticalInt =
          Math.max(
              (int) (Math.round((cardPaddingTopBottomAdj / 2.0) * CARD_SPACING_PCT)),
              0); // round spacing
      int paddingTopBottomInt =
          mGridHeight - ((cardHeightInt * numRows) + (spacingVerticalInt * (numRows - 1)));
      paddingTopInt = Math.max(paddingTopBottomInt / 2, 0);

      int sumSize =
          (cardHeightInt * numRows) + (spacingVerticalInt * (numRows - 1)) + (paddingTopInt * 2);
      if (Math.abs(sumSize - mGridHeight) > 2) {
        Timber.w(
            "setAutoCardGridValues calculation delta > 2, something is off GridHeight <%s> sumSize"
                + " <%s>!",
            mGridHeight, sumSize);
      }
      ...
      spacingHorizontalInt =
          Math.max((int) (Math.round(paddingLeftInt * CARD_SPACING_PCT)), 0); // round spacing
      if (mImageType == ImageType.BANNER) {
        spacingHorizontalInt =
            Math.max(
                (int) (Math.round(paddingLeftInt * CARD_SPACING_HORIZONTAL_BANNER_PCT)),
                0); // round spacing
      }
      int cardsCol =
          (int) Math.round(((double) mGridWidth / (cardWidthInt + spacingHorizontalInt)) + 0.5);
      ...
      spacingHorizontalInt =
          Math.max(
              (int) (Math.round((cardPaddingLeftRightAdj / 2.0) * CARD_SPACING_PCT)),
              0); // round spacing
      if (mImageType == ImageType.BANNER) {
        spacingHorizontalInt =
            Math.max(
                (int)
                    (Math.round(
                        (cardPaddingLeftRightAdj / 2.0) * CARD_SPACING_HORIZONTAL_BANNER_PCT)),
                0); // round spacing
      }
      int paddingLeftRightInt =
          mGridWidth - ((cardWidthInt * numCols) + (spacingHorizontalInt * (numCols - 1)));
      paddingLeftInt = Math.max(paddingLeftRightInt / 2, 0);

      int sumSize =
          (cardWidthInt * numCols) + (spacingHorizontalInt * (numCols - 1)) + (paddingLeftInt * 2);
      if (Math.abs(sumSize - mGridWidth) > 2) {
        Timber.w(
            "setAutoCardGridValues calculation delta > 2, something is off GridWidth <%s> sumSize"
                + " <%s>!",
            mGridWidth, sumSize);
      }
      paddingTopInt = (int) Math.round((cardHeightInt * cardScaling) / 2.0);
      spacingVerticalInt =
          Math.max((int) (Math.round(paddingTopInt * CARD_SPACING_PCT)), 0); // round spacing
      int cardsRow =
          (int) Math.round(((double) mGridHeight / (cardHeightInt + spacingVerticalInt)) + 0.5);
  ...
  private void setupQueries() {
    if (mFolder.getType() == BaseItemKind.USER_VIEW
        || mFolder.getType() == BaseItemKind.COLLECTION_FOLDER) {
      CollectionType type =
          mFolder.getCollectionType() != null
              ? mFolder.getCollectionType()
      ...
      if (type == CollectionType.MUSIC) {
        ...
        if ("AlbumArtist".equals(includeType)) {
          setRowDef(
              new BrowseRowDef(
                  "",
                  BrowsingUtils.createAlbumArtistsRequest(mParentId),
                  CHUNK_SIZE_MINIMUM,
                  new ChangeTriggerType[] {}));
          return;
        } else if ("Artist".equals(includeType)) {
          setRowDef(
              new BrowseRowDef(
                  "",
                  BrowsingUtils.createArtistsRequest(mParentId),
                  CHUNK_SIZE_MINIMUM,
                  new ChangeTriggerType[] {}));
          return;
        }
  ...
  public void onResume() {
    ...
    PosterSize posterSizeSetting =
        libraryPreferences.get(LibraryPreferences.Companion.getPosterSize());
    ImageType imageType = libraryPreferences.get(LibraryPreferences.Companion.getImageType());
    GridDirection gridDirection =
        libraryPreferences.get(LibraryPreferences.Companion.getGridDirection());

    if (mImageType != imageType
      ...
      if (mGridDirection.equals(GridDirection.VERTICAL)
          && (mGridPresenter == null || !(mGridPresenter instanceof VerticalGridPresenter))) {
        setGridPresenter(new VerticalGridPresenter());
      } else if (mGridDirection.equals(GridDirection.HORIZONTAL)
          && (mGridPresenter == null || !(mGridPresenter instanceof HorizontalGridPresenter))) {
  ...
  private void buildAdapter() {
    ...
    Timber.d(
        "buildAdapter cardHeight <%s> getCardWidthBy <%s> chunks <%s> type <%s>",
        mCardHeight,
        (int) getCardWidthBy(mCardHeight, mImageType, mFolder),
        mRowDef.getChunkSize(),
        mRowDef.getQueryType().toString());
    ...
    if (mCardsScreenEst > 0 && mCardsScreenEst >= chunkSize) {
      ...
      Timber.d(
          "buildAdapter adjusting chunkSize to <%s> screenEst <%s>", chunkSize, mCardsScreenEst);
    }
    chunkSize = 100;
    ...
    switch (mRowDef.getQueryType()) {
      case NextUp:
        mAdapter =
            new ItemRowAdapter(
                requireContext(), mRowDef.getNextUpQuery(), true, mCardPresenter, null);
        break;
      case Views:
        mAdapter =
            new ItemRowAdapter(
                requireContext(), GetUserViewsRequest.INSTANCE, mCardPresenter, null);
        break;
      case SimilarSeries:
        mAdapter =
            new ItemRowAdapter(
                requireContext(),
                mRowDef.getSimilarQuery(),
      ...
      case LiveTvChannel:
        mAdapter =
            new ItemRowAdapter(
                requireContext(), mRowDef.getTvChannelQuery(), 40, mCardPresenter, null);
        break;
      case LiveTvProgram:
        mAdapter =
            new ItemRowAdapter(requireContext(), mRowDef.getProgramQuery(), mCardPresenter, null);
        break;
      case LiveTvRecording:
        mAdapter =
            new ItemRowAdapter(
                requireContext(), mRowDef.getRecordingQuery(), chunkSize, mCardPresenter, null);
        break;
      case Artists:
        mAdapter =
            new ItemRowAdapter(
                requireContext(), mRowDef.getArtistsQuery(), chunkSize, mCardPresenter, null);
        break;
      case AlbumArtists:
        mAdapter =
            new ItemRowAdapter(
                requireContext(), mRowDef.getAlbumArtistsQuery(), chunkSize, mCardPresenter, null);
        break;
      default:
        mAdapter =
            new ItemRowAdapter(
                ...
                mRowDef.getQuery(),
                chunkSize,
                mRowDef.getPreferParentThumb(),
                mRowDef.isStaticHeight(),
                mCardPresenter,
                null);
        break;
    }
    ...
    mAdapter.setRetrieveFinishedListener(
        new EmptyResponse() {
          ...
          public void onResponse() {
            ...
            mLetterButton.setVisibility(
                ItemSortBy.SORT_NAME.equals(mAdapter.getSortBy()) ? View.VISIBLE : View.GONE);
  ...
  public void loadGrid() {
    ...
    mAdapter.setSortBy(
        getSortOption(libraryPreferences.get(LibraryPreferences.Companion.getSortBy())));
    mAdapter.Retrieve();
  }
  ...
  private void updateDisplayPrefs() {
    CoroutineUtils.runOnLifecycle(
        getLifecycle(),
        (coroutineScope, continuation) -> {
          libraryPreferences.set(
              LibraryPreferences.Companion.getFilterFavoritesOnly(),
              mAdapter.getFilters().isFavoriteOnly());
          libraryPreferences.set(
              LibraryPreferences.Companion.getFilterUnwatchedOnly(),
              mAdapter.getFilters().isUnwatchedOnly());
          libraryPreferences.set(LibraryPreferences.Companion.getSortBy(), mAdapter.getSortBy());
          libraryPreferences.set(
              LibraryPreferences.Companion.getSortOrder(),
              getSortOption(mAdapter.getSortBy()).order);
  ...
  private void addTools() {
        ...
        new View.OnClickListener() {
          ...
          public void onClick(View v) {
            ...
            for (Map.Entry<Integer, SortOption> entry : sortOptions.entrySet()) {
              MenuItem item =
                  sortMenu.getMenu().add(0, entry.getKey(), entry.getKey(), entry.getValue().name);
              item.setChecked(
                  entry
                      .getValue()
                      .value
                      .equals(libraryPreferences.get(LibraryPreferences.Companion.getSortBy())));
            }
    ...
    if (mRowDef.getQueryType() == QueryType.Items) {
      ...
      mUnwatchedButton.setOnClickListener(
          new View.OnClickListener() {
            @Override
            public void onClick(View v) {
              FilterOptions filters = mAdapter.getFilters();
    ...
    mLetterButton.setOnClickListener(
        new View.OnClickListener() {
          @Override
          public void onClick(View v) {
        ...
        new View.OnClickListener() {
          ...
          public void onClick(View v) {
            boolean allowViewSelection =
                userViewsRepository.getValue().allowViewSelection(mFolder.getCollectionType());
            startActivity(
                ActivityDestinations.INSTANCE.displayPreferences(
                    getContext(), mFolder.getDisplayPreferencesId(), allowViewSelection));
          }
        });
  ...
  class JumplistPopup {
    ...
    JumplistPopup() {
      PopupEmptyBinding layout =
          PopupEmptyBinding.inflate(getLayoutInflater(), binding.rowsFragment, false);
      ...
      alphaPicker.setOnAlphaSelected(
          letter -> {
    ...
    public void show() {
      popupWindow.showAtLocation(
          binding.rowsFragment,
          Gravity.TOP,
          binding.rowsFragment.getLeft(),
          binding.rowsFragment.getTop());
  ...
  private void setupEventListeners() {
    ...
    CoroutineUtils.readCustomMessagesOnLifecycle(
        getLifecycle(),
        customMessageRepository.getValue(),
        message -> {
  ...
  private void refreshCurrentItem() {
    ...
    ItemRowAdapterHelperKt.refreshItem(
        mAdapter,
        api.getValue(),
        this,
        mCurrentItem,
        () -> {
          ...
          if ((mAdapter.getFilters().isFavoriteOnly() && !mCurrentItem.isFavorite())
              || (mAdapter.getFilters().isUnwatchedOnly() && mCurrentItem.isPlayed())) {
            // if we are about to remove the current item, throw focus to toolbar so framework
            // doesn't crash
  ...
  private final Runnable mDelayedSetItem =
      new Runnable() {
        @Override
        public void run() {
          if (!getLifecycle().getCurrentState().isAtLeast(Lifecycle.State.STARTED)) return;
  ...
  private final class ItemViewSelectedListener implements OnItemViewSelectedListener {
    ...
    public void onItemSelected(
      ...
      if (!(item instanceof BaseRowItem)) {
        ...
        if (!determiningPosterSize) mAdapter.loadMoreItemsIfNeeded(mAdapter.indexOf(mCurrentItem));
      }
    }
  }
}
