...
/**
 * Modified ImageCard with no fade on the badge A card view with an {@link ImageView} as its main
 * region.
 */
public class LegacyImageCardView extends BaseCardView {
  ...
  public void setOverlayInfo(BaseRowItem item) {
    ...
    if (getCardType() == BaseCardView.CARD_TYPE_MAIN_ONLY && item.getShowCardInfoOverlay()) {
      switch (item.getBaseItem().getType()) {
        case PHOTO:
          insertCardData(
              item.getBaseItem().getPremiereDate() != null
                  ? DateTimeExtensionsKt.getDateFormatter(getContext())
                      .format(item.getBaseItem().getPremiereDate())
                  : item.getFullName(getContext()),
  ...
  public void setUnwatchedCount(int count) {
    if (count > 0) {
      binding.unwatchedCount.setText(
          count > 99 ? getContext().getString(R.string.watch_count_overflow) : nf.format(count));
