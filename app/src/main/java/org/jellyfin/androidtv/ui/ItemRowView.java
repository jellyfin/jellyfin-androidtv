...
public class ItemRowView extends FrameLayout {
  ...
  public ItemRowView(
      ...
      int ndx,
      RowSelectedListener rowSelectedListener,
      final RowClickedListener rowClickedListener) {
    super(context);
    inflateView(context);
  ...
  public void setItem(BaseItemDto item, int ndx) {
    ...
    if (item.getType() == AUDIO) {
      ...
      String artist =
          item.getArtists() != null && item.getArtists().size() > 0
              ...
              : !TextUtils.isEmpty(item.getAlbumArtist()) ? item.getAlbumArtist() : null;
      ...
      String series =
          item.getSeriesName() != null ? BaseItemExtensionsKt.getFullName(item, mContext) : null;
    ...
    formattedTime =
        TimeUtils.formatMillis(item.getRunTimeTicks() != null ? item.getRunTimeTicks() / 10000 : 0);
  ...
  public void updateWatched() {
    ...
    if (MediaType.VIDEO.equals(mBaseItem.getMediaType())
        && mBaseItem.getUserData() != null
        && mBaseItem.getUserData().getPlayed()) {
