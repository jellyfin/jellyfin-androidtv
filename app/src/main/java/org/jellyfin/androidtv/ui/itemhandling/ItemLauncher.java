...
public class ItemLauncher {
  private final Lazy<NavigationRepository> navigationRepository =
      KoinJavaComponent.<NavigationRepository>inject(NavigationRepository.class);
  private final Lazy<PreferencesRepository> preferencesRepository =
      KoinJavaComponent.<PreferencesRepository>inject(
          org.jellyfin.androidtv.preference.PreferencesRepository.class);
  private final Lazy<MediaManager> mediaManager =
      KoinJavaComponent.<MediaManager>inject(MediaManager.class);
  private final Lazy<PlaybackLauncher> playbackLauncher =
      KoinJavaComponent.<PlaybackLauncher>inject(PlaybackLauncher.class);
  private final Lazy<PlaybackHelper> playbackHelper =
  ...
  public Destination.Fragment getUserViewDestination(@Nullable final BaseItemDto baseItem) {
    CollectionType collectionType =
        baseItem == null ? CollectionType.UNKNOWN : baseItem.getCollectionType();
      ...
      case TVSHOWS:
        ...
        LibraryPreferences displayPreferences =
            preferencesRepository
                .getValue()
                .getLibraryPreferences(baseItem.getDisplayPreferencesId());
        boolean enableSmartScreen =
            displayPreferences.get(LibraryPreferences.Companion.getEnableSmartScreen());
  ...
  public void launch(final BaseRowItem rowItem, ItemRowAdapter adapter, final Context context) {
    switch (rowItem.getBaseRowType()) {
      case BaseItem:
        ...
        switch (baseItem.getType()) {
          ...
          case MUSIC_ARTIST:
            navigationRepository
                .getValue()
                .navigate(Destinations.INSTANCE.itemDetails(baseItem.getId()));
            return;
          ...
          case PLAYLIST:
            navigationRepository
                .getValue()
                .navigate(Destinations.INSTANCE.itemList(baseItem.getId()));
            return;

          case AUDIO:
            if (rowItem.getBaseItem() == null) return;

            // if the song currently playing is selected (and is the exact item - this only happens
            // in the nowPlayingRow), open AudioNowPlayingActivity
            if (mediaManager.getValue().hasAudioQueueItems()
                && rowItem instanceof AudioQueueBaseRowItem
                && rowItem
                    .getBaseItem()
                    .getId()
                    .equals(mediaManager.getValue().getCurrentAudioItem().getId())) {
              navigationRepository.getValue().navigate(Destinations.INSTANCE.getNowPlaying());
            } else if (mediaManager.getValue().hasAudioQueueItems()
                && rowItem instanceof AudioQueueBaseRowItem
                && adapter.indexOf(rowItem) < mediaManager.getValue().getCurrentAudioQueueSize()) {
              ...
              playbackLauncher
                  .getValue()
                  .launch(context, audioItemsAsList, 0, false, adapter.indexOf(rowItem));
            }
          ...
          case BOX_SET:
            navigationRepository
                .getValue()
                .navigate(Destinations.INSTANCE.collectionBrowser(baseItem));
            return;
          ...
          case PHOTO:
            navigationRepository
                .getValue()
                .navigate(
                    Destinations.INSTANCE.pictureViewer(
                        baseItem.getId(), false, adapter.getSortBy(), adapter.getSortOrder()));
            return;
        }
        ...
        if (Utils.isTrue(baseItem.isFolder())) {
          ...
          if (baseItem.getDisplayPreferencesId() == null) {
            baseItem =
                JavaCompat.copyWithDisplayPreferencesId(baseItem, baseItem.getId().toString());
          }
            ...
            case Play:
              ...
              playbackHelper
                  ...
                  .getItemsToPlay(
                      context,
                      baseItem,
                      baseItem.getType() == BaseItemKind.MOVIE,
                      false,
                      new Response<List<BaseItemDto>>() {
                        @Override
      ...
      case Person:
        navigationRepository
            .getValue()
            .navigate(Destinations.INSTANCE.itemDetails(rowItem.getItemId()));
        ...
        switch (rowItem.getSelectAction()) {
          case ShowDetails:
            navigationRepository
                .getValue()
                .navigate(
                    Destinations.INSTANCE.channelDetails(
                        program.getId(), program.getChannelId(), program));
            break;
          case Play:
            // Just play it directly - need to retrieve program channel via items api to convert to
            // BaseItem
            ItemLauncherHelper.getItem(
      ...
      case LiveTvRecording:
        switch (rowItem.getSelectAction()) {
          case ShowDetails:
            navigationRepository
                .getValue()
                .navigate(Destinations.INSTANCE.itemDetails(rowItem.getBaseItem().getId()));
            break;
          case Play:
      ...
      case SeriesTimer:
        navigationRepository
            .getValue()
            .navigate(
                Destinations.INSTANCE.seriesTimerDetails(
                    rowItem.getItemId(),
                    ((SeriesTimerInfoDtoBaseRowItem) rowItem).getSeriesTimerInfo()));
        break;
      ...
      case GridButton:
        switch (((GridButtonBaseRowItem) rowItem).getGridButton().getId()) {
          ...
          case LiveTvOption.LIVE_TV_SERIES_OPTION_ID:
            navigationRepository
                .getValue()
                .navigate(Destinations.INSTANCE.getLiveTvSeriesRecordings());
            break;
