            ...
            public void onResponse(BaseItemDto lastPlayedItem) {
                ...
                playbackManager.getValue().getVideoStreamInfo(mFragment, lastPlayedVideoOptions, 0, new Response<StreamInfo>() {
                    ...
                    public void onResponse(StreamInfo lastPlayedStreamInfo) {
                        ...
                        if (requireNonNull(requireNonNull(lastPlayedMediaSource).getDefaultAudioStreamIndex()) != -1)
                            mPreviousAudio = requireNonNull(lastPlayedMediaSource.getMediaStreams()).get(lastPlayedMediaSource.getDefaultAudioStreamIndex());
                        if (requireNonNull(requireNonNull(lastPlayedMediaSource).getDefaultSubtitleStreamIndex()) != -1)
                            mPreviousSubtitle = requireNonNull(lastPlayedMediaSource.getMediaStreams()).get(lastPlayedMediaSource.getDefaultSubtitleStreamIndex());

                        if (item.getMediaStreams() != null && mPreviousAudio != null)
                            inferredAudioIndex = inferMediaStreamIndex(item, mPreviousAudio);
                        if (item.getMediaStreams() != null && mPreviousSubtitle != null)
