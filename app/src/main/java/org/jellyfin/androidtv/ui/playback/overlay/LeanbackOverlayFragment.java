...
public class LeanbackOverlayFragment extends PlaybackSupportFragment {
    ...
    public void mediaInfoChanged() {
        ...
        long interval = skipForwardLength;//Math.min(skipForwardLength, skipBackLength);
        boolean enableTrickPlay = userPreferences.getValue().get(UserPreferences.Companion.getTrickPlayEnabled());
        playerGlue.setSeekProvider(playerAdapter.canSeek() ? new CustomSeekProvider(playerAdapter, imageLoader.getValue(), api.getValue(), requireContext(), enableTrickPlay, interval) : null);
