...
/**
 * Compose Live TV Library screen with immersive list layout
 * Supports navigation for live TV programs, channels, and recordings
 */
@Composable
fun ComposeLiveTvScreen(
	folderArguments: android.os.Bundle?,
	modifier: Modifier = Modifier,
	viewModel: ComposeLiveTvViewModel = koinViewModel(),
) {
	val uiState by viewModel.uiState.collectAsState()

	// Load data when the screen is first displayed
	LaunchedEffect(folderArguments) {
		viewModel.loadLiveTvData(folderArguments)
	}
...
private fun EmptyScreen(
	...
	Box(
		...
		Column(
			...
			Text(
				text = "No Live TV content available",
				style = MaterialTheme.typography.bodyLarge,
				color = Color.White.copy(alpha = 0.8f),
				textAlign = TextAlign.Center,
			)
...
@Composable
private fun LiveTvContentScreen(
	uiState: LiveTvUiState,
	viewModel: ComposeLiveTvViewModel,
	modifier: Modifier = Modifier,
) {
