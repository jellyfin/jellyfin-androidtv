...
public class BrowseRecordingsFragment extends EnhancedBrowseFragment {
  ...
  protected void setupQueries(final RowLoader rowLoader) {
    ...
    mRows.add(
        new BrowseRowDef(
            ...
            BrowsingUtils.createLiveTVRecordingsRequest(40),
            40));

    // Movies
    BrowseRowDef moviesDef =
        new BrowseRowDef(
            getString(R.string.lbl_movies), BrowsingUtils.createLiveTVMovieRecordingsRequest(), 60);

    // Shows
    BrowseRowDef showsDef =
        new BrowseRowDef(
            getString(R.string.lbl_tv_series),
            BrowsingUtils.createLiveTVSeriesRecordingsRequest(),
            60);
    ...
    mRows.add(
        new BrowseRowDef(
            getString(R.string.lbl_sports),
            BrowsingUtils.createLiveTVSportsRecordingsRequest(),
            60));

    // Kids
    mRows.add(
        new BrowseRowDef(
            getString(R.string.lbl_kids), BrowsingUtils.createLiveTVKidsRecordingsRequest(), 60));
  ...
  private void addNext24Timers() {
    BrowseViewFragmentHelperKt.getLiveTvTimers(
        this,
        timers -> {
          ...
          if (!nearTimers.isEmpty()) {
            ItemRowAdapter scheduledAdapter =
                new ItemRowAdapter(
                    requireContext(), nearTimers, mCardPresenter, mRowsAdapter, true);
            scheduledAdapter.Retrieve();
            ListRow scheduleRow =
                new ListRow(
                    new HeaderItem(getString(R.string.scheduled_in_next_24_hours)),
                    scheduledAdapter);
