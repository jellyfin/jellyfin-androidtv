...
/**
 * Compose Music Library screen with immersive list layout
 * Supports navigation for music albums, artists, and audio content
 */
@Composable
fun ComposeMusicScreen(
	folderArguments: android.os.Bundle?,
	modifier: Modifier = Modifier,
	viewModel: ComposeMusicViewModel = koinViewModel(),
) {
	val uiState by viewModel.uiState.collectAsState()

	// Load data when the screen is first displayed
	LaunchedEffect(folderArguments) {
		viewModel.loadMusicData(folderArguments)
	}
...
private fun ErrorScreen(
	...
	Box(
		...
		Column(
			...
			Text(
				text = "Error Loading Music Library",
...
private fun EmptyScreen(
	...
	Box(
		...
		Column(
			...
			Text(
				text = "No music found in this library",
				style = MaterialTheme.typography.bodyLarge,
				color = Color.White.copy(alpha = 0.8f),
				textAlign = TextAlign.Center,
			)
...
@Composable
private fun MusicContentScreen(
	uiState: MusicUiState,
	viewModel: ComposeMusicViewModel,
	modifier: Modifier = Modifier,
) {
