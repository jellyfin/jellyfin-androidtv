...
public class LiveTvGuideFragment extends Fragment implements LiveTvGuide, View.OnKeyListener {
  ...
  public View onCreateView(
    ...
    programVScroller.setScrollViewListener(
        new ScrollViewListener() {
          @Override
    ...
    mChannelScroller.setScrollViewListener(
        new ScrollViewListener() {
          @Override
          public void onScrollChanged(
              ObservableScrollView scrollView, int x, int y, int oldx, int oldy) {
    ...
    programHScroller.setScrollViewListener(
        new HorizontalScrollViewListener() {
          @Override
          public void onScrollChanged(
              ObservableHorizontalScrollView scrollView, int x, int y, int oldx, int oldy) {
  ...
  public void refreshFavorite(UUID channelId) {
    for (int i = 0; i < mChannels.getChildCount(); i++) {
      ...
      if (gch.getChannel().getId().equals(channelId.toString())) gch.refreshFavorite();
    }
  }
  ...
  private boolean onKeyLongPress(int keyCode) {
    switch (keyCode) {
      case KeyEvent.KEYCODE_ENTER:
      case KeyEvent.KEYCODE_DPAD_CENTER:
        if (mSelectedProgramView instanceof ProgramGridCell) showProgramOptions();
        ...
        if ((event.getFlags() & KeyEvent.FLAG_CANCELED_LONG_PRESS) == 0) {
          if (mSelectedProgramView instanceof ProgramGridCell) {
            if (mSelectedProgram.getStartDate().isBefore(LocalDateTime.now()))
              playbackHelper
                  .getValue()
                  .retrieveAndPlay(mSelectedProgram.getChannelId(), false, requireContext());
            else showProgramOptions();
            return true;
          }
        }
        return false;
  ...
  View.OnClickListener datePickedListener =
      new View.OnClickListener() {
        @Override
        public void onClick(View v) {
  ...
  private void showDatePicker() {
    FrameLayout scrollPane =
        (FrameLayout) getLayoutInflater().inflate(R.layout.horizontal_scroll_pane, null);
    LinearLayout scrollItems = scrollPane.findViewById(R.id.scrollItems);
    for (long increment = 0; increment < 15; increment++) {
      scrollItems.addView(
          new FriendlyDateButton(
              requireContext(), LocalDateTime.now().plusDays(increment), datePickedListener));
    }
  ...
  private void requestGuidePage(final LocalDateTime startTime) {
    new AlertDialog.Builder(requireContext())
        ...
        .setMessage(
            startTime.isAfter(mCurrentGuideStart)
  ...
  public void showProgramOptions() {
    ...
    if (mDetailPopup == null) {
      mDetailPopup =
          new LiveProgramDetailPopup(
              requireActivity(),
              this,
              this,
              mSummary.getWidth() + 20,
              new EmptyResponse() {
                @Override
    ...
    TvManager.getProgramsAsync(
        ...
        new EmptyResponse() {
          ...
          public void onResponse() {
            ...
            mDisplayProgramsTask.execute(
                mCurrentDisplayChannelStartNdx, mCurrentDisplayChannelEndNdx);
          }
        });
  }
  ...
  class DisplayProgramsTask extends AsyncTask<Integer, Integer, Void> {
    ...
    protected void onPreExecute() {
      ...
      if (mCurrentDisplayChannelStartNdx > 0) {
        ...
        String label =
            TextUtilsKt.getLoadChannelsLabel(
                requireContext(),
                mAllChannels.get(pageUpStart).getNumber(),
                mAllChannels.get(mCurrentDisplayChannelStartNdx - 1).getNumber());
        mProgramRows.addView(
            new GuidePagingButton(requireContext(), LiveTvGuideFragment.this, pageUpStart, label));
      }
    }
    ...
    protected void onPostExecute(Void aVoid) {
      ...
      if (mCurrentDisplayChannelEndNdx < mAllChannels.size() - 1 && !mFilters.any()) {
        ...
        String label =
            TextUtilsKt.getLoadChannelsLabel(
                requireContext(),
                mAllChannels.get(mCurrentDisplayChannelEndNdx + 1).getNumber(),
                mAllChannels.get(pageDnEnd).getNumber());
        mProgramRows.addView(
            new GuidePagingButton(
                requireContext(),
                LiveTvGuideFragment.this,
                mCurrentDisplayChannelEndNdx + 1,
                label));
      }
      ...
      mResetButton.setVisibility(
          mCurrentGuideStart.isAfter(LocalDateTime.now())
              ? View.VISIBLE
              : View.GONE); // show reset button if paged ahead
  ...
  private GuideChannelHeader getChannelHeader(
      Context context, org.jellyfin.sdk.model.api.BaseItemDto channel) {
  ...
  private LinearLayout getProgramRow(List<BaseItemDto> programs, UUID channelId) {
    ...
    if (programs.size() == 0) {
      ...
      int minutes =
          ((Long)
                  ((mCurrentGuideEnd.toInstant(ZoneOffset.UTC).toEpochMilli()
                          - mCurrentGuideStart.toInstant(ZoneOffset.UTC).toEpochMilli())
                      / 60000))
              .intValue();
      int slot = 0;
      do {
        BaseItemDto empty =
            LiveTvGuideFragmentHelperKt.createNoProgramDataBaseItem(
                getContext(),
                channelId,
        ...
        cell.setLayoutParams(
            new ViewGroup.LayoutParams(30 * guideRowWidthPerMinPx, guideRowHeightPx));
    ...
    for (BaseItemDto item : programs) {
      LocalDateTime start =
          item.getStartDate() != null ? item.getStartDate() : getCurrentLocalStartDate();
      ...
      if (start.isAfter(prevEnd)) {
        ...
        BaseItemDto empty =
            LiveTvGuideFragmentHelperKt.createNoProgramDataBaseItem(
                getContext(), channelId, prevEnd, start);
            ...
            new ViewGroup.LayoutParams(
                ((Long)
                            ((start.toInstant(ZoneOffset.UTC).toEpochMilli()
                                    - prevEnd.toInstant(ZoneOffset.UTC).toEpochMilli())
                                / 60000))
                        .intValue()
                    * guideRowWidthPerMinPx,
                guideRowHeightPx));
      ...
      if (duration > 0) {
        ...
        program.setLayoutParams(
            new ViewGroup.LayoutParams(
                duration.intValue() * guideRowWidthPerMinPx, guideRowHeightPx));
    ...
    if (prevEnd.isBefore(mCurrentGuideEnd)) {
      ...
      BaseItemDto empty =
          LiveTvGuideFragmentHelperKt.createNoProgramDataBaseItem(
              getContext(), channelId, prevEnd, mCurrentGuideEnd);
  ...
  void detailUpdateInternal() {
    ...
    mDisplayDate.setText(
        TimeUtils.getFriendlyDate(requireContext(), mSelectedProgram.getStartDate()));
    String url =
        imageHelper
            .getValue()
            .getPrimaryImageUrl(mSelectedProgram, null, ImageHelper.MAX_PRIMARY_IMAGE_HEIGHT);
    mImage.load(
        url, null, ContextCompat.getDrawable(requireContext(), R.drawable.blank10x10), 0, 0);
        ...
        if (mSelectedProgramView == mChannels.getChildAt(i)) {
          ...
          for (int ii = 0; ii < programRow.getChildCount(); ii++) {
            ...
            if (prog.getProgram() != null
                && prog.getProgram().getStartDate().isBefore(LocalDateTime.now())
                && prog.getProgram().getEndDate().isAfter(LocalDateTime.now())) {
