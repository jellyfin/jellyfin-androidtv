...
class LibraryDreamService : DreamService() {
	companion object {
		...
		const val UPDATE_DELAY = 3_000L // 30s
		const val TRANSITION_DURATION = 1_000L // 1s
	}

	private val scope = CoroutineScope(Dispatchers.IO + SupervisorJob())
	private val apiClient: KtorClient by inject(systemApiClient)
	...
	private suspend fun getRandomItem(): BaseItemDto? = try {
		val response by itemsApi.getItemsByUserId(
