    ...
    private void playInternal(final BaseItemDto item, final long position, final VideoOptions options) {
        ...
        mApplication.getPlaybackManager().getVideoStreamInfo(apiClient.getServerInfo().getId(), options, false, apiClient, new Response<StreamInfo>() {
            ...
            public void onResponse(StreamInfo response) {
                ...
                if (!mVideoManager.isNativeMode() && response.getMediaSource().getDefaultAudioStream() != null && (response.getMediaSource().getDefaultAudioStream().getChannels() <= 2 || (!Utils.isGreaterThan51() && "ac3".equals(response.getMediaSource().getDefaultAudioStream().getCodec())))) {
